...
set.seed(1)
od=file.path("C:/Users/18904/Github/huashu-cup","解答","4-第四题");bp=file.path("C:/Users/18904/Github/huashu-cup","解答","0-数据预处理");ap=file.path("C:/Users/18904/Github/huashu-cup","题目","附件数据")
...
sched=function(lc=1,pr=NULL,W=48,K=5,lt=1){if(is.null(pr))pr=price
 ecm=pr+lc*ci;cvm=apply(ecm,2,function(v)c(0,cumsum(v)))
 gocc=matrix(0,TT,length(R));colnames(gocc)=R;aip=gocc;sch=data.frame();n_mig=0;t_del=0
 ord=c(which(d$任务类型=="实时推理任务"),which(d$任务类型!="实时推理任务"))
 for(i in ord){g=as.numeric(d[i,"图形处理器需求量"]);P=d$P[i];L=d$L[i];at=d$到达小时[i];typ=d$任务类型[i];src=d$来源区域[i]
  if(typ=="实时推理任务"){reg=src;s=at;e=as.numeric(d[i,"最晚完成小时"])}
  else{dl=as.numeric(d[i,"最晚完成小时"]);cand=names(which(lm[src,]<=as.numeric(d[i,"最大网络时延"])));best=NULL
   for(r in cand){hi=min(at+W,dl-L);if(hi<at)next
...
stor=function(net,price,E,Smin,S0,Pc,Pd,ec,ed,Smax){win=24;tg=as.numeric(stats::filter(net,rep(1/win,win),sides=2));tg[is.na(tg)]=net[is.na(tg)]
 ...
 for(t in 1:n){if(net[t]<0){sc=min(Pc,-net[t],(E-soc)/ec);soc=soc+ec*sc;pg[t]=0;ch[t]=sc}
...
run=function(cp=50,pr=NULL,dire_scale=1,tsk=NULL){if(is.null(pr))pr=price
 if(is.null(tsk))tsk=sched(cp,pr)
 aip=tsk$aip;cost=carbon=0;peak=0;vari=0
 for(r in R){q=rt[rt$区域编号==r,];s=st[st$区域编号==r,]
  fac=(aip[,r]+q$非人工智能信息技术负荷)*pue[r];net=fac-q$直接消纳新能源*dire_scale
  st2=stor(net,pr[,r],s$储能容量,s$最小荷电状态,s$初始荷电状态,s$最大充电功率,s$最大放电功率,s$充电效率,s$放电效率,s$外送上限)
  cost=cost+sum(st2$pg*pr[,r]);carbon=carbon+sum(st2$pg*ci[,r])
  peak=max(peak,max(st2$pg));vari=vari+sd(st2$pg)}
 list(m=c(购电成本万元=round(cost/1e4,1),碳排放吨=round(carbon,0),迁移时延_ms=round(tsk$t_del,0),迁移任务数=tsk$n_mig,峰值净购电_MW=round(peak,1),负荷波动=round(vari,2)),tsk=tsk)}
...
res=data.frame(场景=c("基准","碳约束","电价机制","新能源波动"),购电成本万元=c(s1$m[1],s2$m[1],s3$m[1],s4$m[1]),碳排放吨=c(s1$m[2],s2$m[2],s3$m[2],s4$m[2]),迁移时延_ms=c(s1$m[3],s2$m[3],s3$m[3],s4$m[3]),迁移任务数=c(s1$m[4],s2$m[4],s3$m[4],s4$m[4]),新能源利用率=c(s1$m[5],s2$m[5],s3$m[5],s4$m[5]),峰值净购电_MW=c(s1$m[6],s2$m[6],s3$m[6],s4$m[6]),负荷波动=c(s1$m[7],s2$m[7],s3$m[7],s4$m[7]))
