# ===== 问题三 储能协同优化：峰谷套利+削峰填谷+新能源消纳 =====
# 依据文献：Bi-Level共享储能/樊小朝源网荷储算协同/付智削峰填谷/王一航新能源消纳
# 给定任务调度与逐时负荷(基准AI+非AI IT负荷)×PUE；仅优化储能充放电、购售电、新能源分配
# 对比"无储能"与"有储能"：购电成本、碳排放、峰值净购电、负荷波动
library(readr);library(dplyr);library(ggplot2)
od=file.path("C:/Users/18904/Github/huashu-cup","解答","3-第三题")
...
# ---- 无储能基准：净负荷(负荷-直接消纳新能源)直接购电，富余外送/弃 ----
base=function(net,sell,Smax){
 pg=pmax(net,0);rem=pmax(-net,0);ex=pmin(Smax,rem);cur=pmax(rem-ex,0)
 list(m=c(成本=sum(pg*price)-sum(ex*sell),碳=sum(pg*ci),峰值=max(pg-ex),波动=sd(pg-ex)),nc=pg-ex)
}
# ---- 有储能：移动平均削峰填谷 + 宽松电价过滤套利 + 新能源富余充电 + 终端SOC约束 ----
stor=function(net,price,sell,ci,E,Smin,S0,Pc,Pd,ec,ed,Smax){
 ...
 for(t in 1:n){
  if(net[t]<0){sc=min(Pc,-net[t],(E-soc)/ec);soc=soc+ec*sc;rem=-net[t]-sc
 ...
 if(soc<S0)for(t in n:1){if(soc>=S0)break;gc=min(Pc,(S0-soc)/ec,(E-soc)/ec);if(gc<=0)next
   soc=soc+ec*gc;ch[t]=ch[t]+gc;pg[t]=pg[t]+gc}   # 终端SOC补齐(不低于初始)
...
for(r in R){
 s=st[st$区域编号==r,];q=rt[rt$区域编号==r,]
 fac=(q$基准人工智能信息技术负荷+q$非人工智能信息技术负荷)*pue[r]
 net=fac-q$直接消纳新能源     # 净负荷=设施负荷-新能源直接消纳
 price=q$购电价格;sell=q$售电价格;ci=q$碳强度
 b=base(net,sell,s$外送上限)
 st2=stor(net,price,sell,ci,s$储能容量,s$最小荷电状态,s$初始荷电状态,
      s$最大充电功率,s$最大放电功率,s$充电效率,s$放电效率,s$外送上限)
 cat("区域",r," 充电量=",round(st2$ch,0)," 放电量=",round(st2$ds,0),
     " 终SOC=",st2$m[5],"\n")
 ...
 alldat=rbind(alldat,data.frame(区域=r,小时=0:2406,方案="无储能",净购电=b$nc),
                     data.frame(区域=r,小时=0:2406,方案="有储能",净购电=st2$nc))
}
print(out)
write_csv(out,file.path(od,"问题三_指标对比.csv"))
g2=ggplot(alldat[alldat$小时<=336,],aes(x=小时,y=净购电,color=方案,linetype=方案))+geom_line(linewidth=.6)+
 scale_color_manual(values=c("无储能"="grey45","有储能"="#2166AC"))+
 scale_linetype_manual(values=c("无储能"="dashed","有储能"="solid"))+
 facet_wrap(~区域,ncol=2,scales="fixed")+    # 纵坐标统一,便于横向对比振幅
 labs(x="小时(第0–336小时)",y="净购电功率(MW)",color="方案",linetype="方案")+
