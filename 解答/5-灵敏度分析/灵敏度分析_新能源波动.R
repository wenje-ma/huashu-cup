# 灵敏度C：新能源出力降低梯度(保留90%/80%/70%/60%)对系统指标的影响（问题3口径+储能）
...
stor=function(net,price,E,Smin,S0,Pc,Pd,ec,ed,Smax){
 ...
 pg=ds=ch=numeric(n)
 for(t in 1:n){
  if(net[t]<0){sc=min(Pc,-net[t],(E-soc)/ec);soc=soc+ec*sc;pg[t]=0}
...
sc=c(1,.9,.8,.7,.6);res=data.frame()   # 新能源保留比例
for(k in sc){
 ...
 for(r in R){
  ...
  net=fac-q$直接消纳新能源*k
  pg=stor(net,price[,r],s$储能容量,s$最小荷电状态,s$初始荷电状态,
       s$最大充电功率,s$最大放电功率,s$充电效率,s$放电效率,s$外送上限)
  cost=cost+sum(pg*price[,r]);carbon=carbon+sum(pg*ci[,r])
  peak=max(peak,max(pg));vari=vari+sd(pg)
 }
 res=rbind(res,data.frame(新能源保留=k,购电成本万元=round(cost/1e4,1),碳排放吨=round(carbon,0),峰值净购电_MW=round(peak,1),负荷波动=round(vari,2)))
}
write_csv(res,file.path(od,"灵敏度_新能源波动.csv"));print(res)
cat("输出目录:",od,"\n")
